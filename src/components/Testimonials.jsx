...
export default function Testimonials() {
  const [visible, setVisible] = useState(() => isMobile() ? PAGE_MOBILE : PAGE_DESKTOP);
  const page = isMobile() ? PAGE_MOBILE : PAGE_DESKTOP;
        ...
        <div style={{ position: 'relative' }}>
          ...
          >
            {TESTIMONIALS.slice(0, visible).map((x, i) => (
              <figure
                key={i}
                className={`testimonial-card anim-fade-up${inView ? ' is-visible' : ''}`}
